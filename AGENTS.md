...
Path map: `home/` syncs to `~/.codex/` · `home/config.toml` is merged into `~/.codex/config.toml` instead of being copied verbatim · `local/` holds machine-specific templates (`*.example`) and ignored real overrides.
