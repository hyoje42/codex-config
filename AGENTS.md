...
- **Sync-script validation**: `codex-diff-with-home` is a thin wrapper for `codex-sync-to-home --dry-run`; all plan/compare logic (config merge, wrapper decision, legacy/retired skill detection) lives in `codex-sync-to-home`. After changing Codex skill path or sync behavior, run `for script in codex-sync-to-home tests/*.sh; do bash -n "$script"; done`, `./tests/sync-home.sh`, and `./tests/sync-skill-paths.sh` (both use a temporary HOME; never touch `~/.codex`, `~/.agents`, or `~/.bashrc`).
